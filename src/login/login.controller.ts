...
export class loginController{
    ...
    @Get('oauth')
    OAuthController(@Req() req: Request , @Res() res: Response , @Query('code') auth_code: String){
        this.loginService.executeOAuth(req , res , auth_code)
    }

    @Get('user')
    getUserController(@Req() req: Request , @Res() res: Response , @Query('email') email: String){
